...
!
     ...
     &                      BOUNDARY(ng) % v_west,                      &
     &                      update)
            IF (FoundError(exit_flag, NoError, 1180, MyFile)) RETURN
          END IF
!
     ...
     &                      BOUNDARY(ng) % u_east,                      &
     &                      update)
            IF (FoundError(exit_flag, NoError, 1189, MyFile)) RETURN
          END IF
!
...
!
     ...
     &                      BOUNDARY(ng) % v_south,                     &
     &                      update)
            IF (FoundError(exit_flag, NoError, 1216, MyFile)) RETURN
          END IF
!
...
!
     ...
     &                       CLIMA(ng)%ubarclm,                         &
     &                       update)
        IF (FoundError(exit_flag, NoError, 1344, MyFile)) RETURN
!
