...
!
     ...
     &                   GRID(ng) % pmask_wet,                          &
     &                   SetFillVal = .FALSE.)
      IF (FoundError(status, nf90_noerr, 192, MyFile)) THEN
        IF (Master) THEN
...
!
     ...
     &                   GRID(ng) % umask_wet,                          &
     &                   SetFillVal = .FALSE.)
      IF (FoundError(status, nf90_noerr, 234, MyFile)) THEN
        IF (Master) THEN
...
!
     ...
     &                   GRID(ng) % vmask_wet,                          &
     &                   SetFillVal = .FALSE.)
      IF (FoundError(status, nf90_noerr, 255, MyFile)) THEN
        IF (Master) THEN
...
!
     ...
     &                   OCEAN(ng) % ubar_stokes)
        IF (FoundError(status, nf90_noerr, 1169, MyFile)) THEN
...
!
     ...
     &                   OCEAN(ng) % vbar_stokes)
        IF (FoundError(status, nf90_noerr, 1188, MyFile)) THEN
...
!
     ...
     &                   GRID(ng) % umask,                              &
     &                   OCEAN(ng) % u_stokes)
        IF (FoundError(status, nf90_noerr, 1208, MyFile)) THEN
...
!
     ...
     &                   GRID(ng) % vmask,                              &
     &                   OCEAN(ng) % v_stokes)
        IF (FoundError(status, nf90_noerr, 1227, MyFile)) THEN
...
!
      ...
      IF (FoundError(exit_flag, NoError, 1242, MyFile)) RETURN
