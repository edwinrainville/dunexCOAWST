...
!
!svn $Id: def_rst.F 1054 2021-03-06 19:47:12Z arango $
!================================================== Hernan G. Arango ===
...
!
      DEFINE : IF (Ldefine) THEN
        ...
        IF (FoundError(exit_flag, NoError, 118, MyFile)) THEN
...
!
     ...
     &                 nf90_unlimited, DimIDs(12))
        IF (FoundError(exit_flag, NoError, 293, MyFile)) RETURN
        recdim=DimIDs(12)
!
...
!
     ...
     &                 SetFillVal = .FALSE.)
        IF (FoundError(exit_flag, NoError, 706, MyFile)) RETURN
!
...
!
     ...
     &                   NF_FRST, nvd4, t3dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1068, MyFile)) RETURN
        END DO
!
...
!
     ...
     &                 NF_FRST, nvd4, r3dgrd, Aval, Vinfo, ncname)
        IF (FoundError(exit_flag, NoError, 1150, MyFile)) RETURN
!
...
!
     ...
     &                 NF_FRST, nvd4, w3dgrd, Aval, Vinfo, ncname,      &
     &                 SetFillVal = .FALSE.)
        IF (FoundError(exit_flag, NoError, 1356, MyFile)) RETURN
!
...
!
     ...
     &                 NF_FRST, nvd3, su2dgrd, Aval, Vinfo, ncname)
        IF (FoundError(exit_flag, NoError, 1647, MyFile)) RETURN
!
...
!
     ...
     &                 NF_FRST, nvd3, sv2dgrd, Aval, Vinfo, ncname)
        IF (FoundError(exit_flag, NoError, 1668, MyFile)) RETURN
!
...
!
     ...
     &                 NF_FRST, nvd4, su3dgrd, Aval, Vinfo, ncname)
        IF (FoundError(exit_flag, NoError, 1691, MyFile)) RETURN
!
...
!
     ...
     &                 NF_FRST, nvd4, w3dgrd, Aval, Vinfo, ncname)
        IF (FoundError(exit_flag, NoError, 1755, MyFile)) RETURN
!
!-----------------------------------------------------------------------
...
!
        ...
        IF (FoundError(exit_flag, NoError, 1764, MyFile)) RETURN
!
!-----------------------------------------------------------------------
...
!
        ...
        IF (FoundError(exit_flag, NoError, 1771, MyFile)) RETURN
      END IF DEFINE
!
!=======================================================================
...
!
        ...
        IF (FoundError(exit_flag, NoError, 1786, MyFile)) THEN
...
!
     ...
     &                         ncid = RST(ng)%ncid)
        IF (FoundError(exit_flag, NoError, 1795, MyFile)) RETURN
!
!  Inquire about the variables.
!
        CALL netcdf_inq_var (ng, iNLM, ncname,                          &
     &                       ncid = RST(ng)%ncid)
        IF (FoundError(exit_flag, NoError, 1801, MyFile)) RETURN
!
