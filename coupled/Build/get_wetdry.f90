...
!
!svn $Id: get_wetdry.F 1054 2021-03-06 19:47:12Z arango $
!================================================== Hernan G. Arango ===
...
!-----------------------------------------------------------------------
!
      IF (FoundError(exit_flag, NoError, 62, MyFile)) RETURN
      ncname=INI(ng)%name
!
...
!
      IF (INI(ng)%ncid.eq.-1) THEN
        ...
        IF (FoundError(exit_flag, NoError, 69, MyFile)) THEN
...
!
     ...
     &                       ncid = INI(ng)%ncid)
      IF (FoundError(exit_flag, NoError, 79, MyFile)) RETURN
!
...
!
     ...
     &                  GRID(ng) % pmask,                               &
     &                  GRID(ng) % pmask_wet)
      IF (FoundError(status, nf90_noerr, 153, MyFile)) THEN
...
!
     ...
     &                  GRID(ng) % rmask,                               &
     &                  GRID(ng) % rmask_wet)
      IF (FoundError(status, nf90_noerr, 176, MyFile)) THEN
...
!
     ...
     &                  GRID(ng) % umask,                               &
     &                  GRID(ng) % umask_wet)
      IF (FoundError(status, nf90_noerr, 199, MyFile)) THEN
...
!
     ...
     &                  GRID(ng) % vmask,                               &
     &                  GRID(ng) % vmask_wet)
      IF (FoundError(status, nf90_noerr, 222, MyFile)) THEN
